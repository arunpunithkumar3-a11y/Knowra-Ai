...
class BuisnessService:
    async def get_business_by_id(
        self, business_id: UUID, session: AsyncSession
    ) -> Optional[Business]:
    ...
    async def get_businesses_by_owner(
        self, owner_id: UUID, session: AsyncSession, skip: int = 0, limit: int = 100
    ) -> list[Business]:
        result = await session.execute(
            select(Business)
    ...
    async def create_business(
        self, business_data: BusinessCreate, owner_id: UUID, session: AsyncSession
    ) -> Business:
    ...
    async def update_business(
        ...
        target_business = await self.get_business_by_id(business_id, session)
